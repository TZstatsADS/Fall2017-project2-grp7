...
shinyServer(
  ...
  function(input, output) 
  {
    ...
    output$uiEda3 <- renderUI({
      ...
      selectInput( 'eda3BikeNeighborhood', 'Choose Neighborhood', 
                   choices = c("ALL", unique(air$geo_entity_name)), selected = "Manhattan")
    ...
    ## render bike stations analysis table
    
    output$tableBikestations <- renderDataTable(distance(data = bikeStations ,loc = geocodeAdddress(input$eda3BikeNeighborhood), km = input$dis))
